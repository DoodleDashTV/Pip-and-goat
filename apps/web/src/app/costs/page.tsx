...
export default async function Page() {
  ...
  const rows = await prisma.costLedgerEntry.findMany({ orderBy: { createdAt: 'desc' }, take: 40 });
  const items = rows.length
    ? rows.map((row) => ({ id: row.id, title: row.category, subtitle: row.notes ?? '', badge: String(row.amountUnits) }))
    : [{ id: 'empty', title: 'No ledger entries yet', subtitle: 'Cost director estimates appear during production planning.', badge: '0' }];
