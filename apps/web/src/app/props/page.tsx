...
export default async function Page() {
  ...
  const rows = await prisma.prop.findMany({ orderBy: { name: 'asc' } });
  const items = rows.map((row) => ({
    id: row.id,
    title: row.name,
    subtitle: row.description ?? row.internalCode,
    badge: `${row.condition} · ${row.status}`,
  }));
  ...
  return (
    <div className="space-y-6">
      <header>
        ...
        <h1 className="mt-2 font-display text-4xl font-bold">Props</h1>
      </header>
