...
import {
  ...
  GOAT_V4_REQUIRED_RENDER_ASSET_SHA256,
  GOAT_V4_REQUIRED_RENDER_CODE_SHA256,
  GOAT_V4_REQUIRED_SOURCE_COMMIT,
  GoatV4PaidMutationTripwire,
  consumeGoatV4Authorization,
  createGoatV4AuthorizationReceipt,
  evaluateGoatV4PaidExecutionAuthorization,
  readGoatV4ConsumptionLedger,
} from './index';

function makePinnedRepo(): string {
  const root = mkdtempSync(path.join(os.tmpdir(), 'goat-v4-pin-'));
  const config = path.join(root, 'config/cloud');
  mkdirSync(config, { recursive: true });
  const ref = `ghcr.io/doodledashtv/ddp-runpod-blender@${GOAT_V4_REQUIRED_DIGEST}`;
  writeFileSync(
    ...
    `${JSON.stringify({
      ...
      ref,
      sourceCommit: GOAT_V4_REQUIRED_SOURCE_COMMIT,
...
describe('Goat paid execution V4', () => {
  ...
  it('authorizes only when the image, source, receipt, price, Pod count, and durable output proof all match', () => {
    ...
    const live = {
      ...
      registryOk: true,
      registryDigest: GOAT_V4_REQUIRED_DIGEST,
      registrySourceCommit: GOAT_V4_REQUIRED_SOURCE_COMMIT,
      registryRenderCodeSha256: GOAT_V4_REQUIRED_RENDER_CODE_SHA256,
      registryRenderAssetSha256: GOAT_V4_REQUIRED_RENDER_ASSET_SHA256,
      registryCapabilitySchema: 'TIVVLEJOY_CHARACTER_WORKER_CAPABILITY_V2',
      registryEntrypoint: 'TIVVLEJOY_CHARACTER_MASTER_DISPATCH_V4',
      registryRequiredAuthorization: GOAT_LIVE_PAID_EXECUTION_AUTHORIZATION_V4,
    ...
    const allowed = evaluateGoatV4PaidExecutionAuthorization({
      repoRoot,
      live,
    ...
    const wrongDigest = evaluateGoatV4PaidExecutionAuthorization({
      repoRoot,
      live: { ...live, registryDigest: `sha256:${'f'.repeat(64)}` },
      nowMs: Date.parse('2026-08-24T23:00:00.000Z'),
    });
    expect(wrongDigest.launchAllowed).toBe(false);
    expect(wrongDigest.remainingBlockers).toContain('V4_IMAGE_PROVENANCE_MISMATCH');

    const expired = evaluateGoatV4PaidExecutionAuthorization({
      repoRoot,
      live,
      nowMs: Date.parse('2026-08-26T00:00:00.000Z'),
    });
    expect(expired.remainingBlockers).toContain('V4_AUTHORIZATION_EXPIRED');
  });

  it('consumes the V4 ledger atomically and permanently forbids a second CREATE', () => {
    const root = mkdtempSync(path.join(os.tmpdir(), 'goat-v4-ledger-'));
    expect(readGoatV4ConsumptionLedger(root)).toBeNull();
    const first = consumeGoatV4Authorization(root, new Date('2026-08-24T23:00:00.000Z'));
    expect(first.ok).toBe(true);
    expect(readGoatV4ConsumptionLedger(root)).toMatchObject({
      consumed: true,
      createAttempted: true,
      createRequestOrdinal: 1,
      plannedPodName: GOAT_V4_PLANNED_POD_NAME,
    });
    const second = consumeGoatV4Authorization(root, new Date('2026-08-24T23:01:00.000Z'));
  ...
  it('keeps preflight read-only and gives the launcher one CREATE plus mandatory teardown', () => {
    const preflight = readFileSync(
      path.resolve(__dirname, '../../../../../scripts/cloud/goat-paid-execution-v4/preflight.ts'),
      'utf8',
    );
    const launch = readFileSync(
      path.resolve(__dirname, '../../../../../scripts/cloud/goat-paid-execution-v4/launch.ts'),
      'utf8',
    );
