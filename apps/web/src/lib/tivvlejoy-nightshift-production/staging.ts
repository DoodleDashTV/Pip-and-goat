...
export function planCharacterStaging(input: {
  ...
  const body = {
    ...
    foregroundCharacter: goatZone.z < pipZone.z ? 'GOAT' : pipZone.z < goatZone.z ? 'PIP' : 'NONE',
    backgroundCharacter: goatZone.z > pipZone.z ? 'GOAT' : pipZone.z > goatZone.z ? 'PIP' : 'NONE',
    crossing: false,
    screenDirection: travel === 'NONE' ? 'NEUTRAL' : travel,
