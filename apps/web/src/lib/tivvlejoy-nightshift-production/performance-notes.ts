...
export function buildDirectorPerformanceNote(input: {
  ...
  const body = {
    ...
    gazeTargetImportance: (input.mode === 'MAP_READING' || input.beat.prop ? 'HIGH' : speaking ? 'MEDIUM' : 'HIGH') as DirectorPerformanceNote['gazeTargetImportance'],
    gestureSize: (input.characterId === 'PIP' ? 'MEDIUM' : comedy ? 'LARGE' : 'SMALL') as DirectorPerformanceNote['gestureSize'],
    movementUrgency: (input.beat.beatType === 'MOVEMENT' ? 'HIGH' : input.beat.beatType === 'TENSION' ? 'MEDIUM' : 'LOW') as DirectorPerformanceNote['movementUrgency'],
