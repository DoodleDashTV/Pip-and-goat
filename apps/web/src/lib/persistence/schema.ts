...
export const previewWorkspaceSchema: z.ZodType<PreviewWorkspace> = z.object({
  ...
  renderRequests: z.array(
    z.object({
      ...
      progress: z.null(),
      createdAt: z.string().min(1).max(80),
      readinessStatus: z.string().max(80).optional(),
      readinessCard: z
        .object({
          episodeLabel: z.string().max(80),
          shotLabel: z.string().max(80),
          status: z.string().max(80),
          backendProven: z.boolean(),
          hashesVerified: z.boolean(),
          assetsApprovedLabel: z.string().max(80),
          shotApprovalLabel: z.string().max(120),
          cacheLabel: z.string().max(80),
          estimatedRuntimeLabel: z.string().max(40),
          gpuLabel: z.string().max(80),
          hourlyQuoteLabel: z.string().max(40),
          estimatedComputeLabel: z.string().max(40),
          maximumCostLabel: z.string().max(40),
          providerContacted: z.literal(false),
          gpuLaunched: z.literal(false),
          paidAuthorization: z.literal('REQUIRED'),
          blockingReason: z.string().max(400).nullable(),
        })
        .optional(),
    }),
  ).max(20),
