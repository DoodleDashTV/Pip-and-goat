...
describe('object keys, filenames, and allowlists', () => {
  ...
  it('keeps the expected official inventory at 14 files and 4 collections', () => {
    expect(assertInventoryCounts()).toEqual({
      sourceCount: EXPECTED_SOURCE_COUNT,
      collectionCount: EXPECTED_COLLECTION_COUNT,
    });
    expect(listExpectedSourceFiles()).toHaveLength(14);
  });
});
...
describe('one-tap purchased selection review', () => {
  it('auto-maps the exact saved filenames and known download-name variants', () => {
    const review = reviewOneTapPurchasedSelection([
      ...
      { filename: 'Stylized_Forest_Nature_Kit.zip', byteSize: 107_061_098 },
    ]);
  ...
  it('includes the three confirmed purchase-site files in the official 14-file count', () => {
    const review = reviewOneTapPurchasedSelection([
    ...
    expect(review.expectedCount).toBe(14);
  ...
  it('maps mixed exact filenames into all four collections and refuses others individually', () => {
    const review = reviewOneTapPurchasedSelection([
      ...
      { filename: 'Giveaway_World Shaders.zip', byteSize: 1024 },
    ]);
    expect(review.checkpoint).toBe(ONE_TAP_UPLOAD_CHECKPOINT);
    expect(review.expectedCount).toBe(14);
    expect(new Set(review.matched.map((item) => item.collectionId))).toEqual(
      new Set(['village', 'sky-hdri', 'stylized-forest', 'world-shaders']),
    );
    expect(review.eligible).toHaveLength(4);
    expect(review.duplicates).toHaveLength(1);
    expect(review.incorrect[0]?.filename).toBe('village blender');
    expect(review.unexpected.some((item) => item.filename === 'not-a-purchased-file.exe')).toBe(true);
    expect(review.unexpected.some((item) => item.filename === 'Rock_Models.blend')).toBe(true);
    ...
    expect(review.missing).toHaveLength(10);
    ...
    expect(
      review.collectionTotals.find((item) => item.collectionId === 'world-shaders')?.bytes,
    ).toBe(1024);
    expect(review.overallTotals.expected).toBe(14);
    expect(review.overallTotals.eligible).toBe(4);
  });
});
...
describe('workspace readiness and git safety', () => {
  it('keeps software foundation separate from real asset readiness', () => {
    ...
    expect(snapshot.realAssetReadiness.expectedFiles).toBe(14);
  ...
  it('keeps TivvleJoy intake copy in the scenery workspace and omits licensed binaries from Git', () => {
    ...
    expect(intake).toContain('Expected 14-file source checklist');
