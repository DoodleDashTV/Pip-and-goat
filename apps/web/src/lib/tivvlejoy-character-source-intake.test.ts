...
describe('Goat character source intake bridge', () => {
  ...
  it('refuses TIVVLEJOY_GOAT_REAL_PAID_EXECUTION_AUTHORIZATION_V1 when the digest-pinned worker cannot be resolved', () => {
    ...
    expect(decision.remainingBlockers).toContain('LIVE_AUTHORIZATION_V6_REQUIRED');
  ...
  it('resolves RUNPOD_WORKER_IMAGE from the authoritative character pin and rejects stale digests', () => {
    ...
    const provenDigest = 'sha256:78f1b16286b0ffc331a787d74297593fa98c455fb24fcecf48d5be5bdeec48b6';
    expect(pin.digest).toBe(provenDigest);
    expect(pin.sourceCommit).toBe('db8658c835ccb0e5a3adc3201d7ba90e9aaeaa46');
  ...
  it('accepts the proven Capability V2/V6 digest and rejects superseded digests, tags, and V1-V5 auths', () => {
    const provenDigest = 'sha256:78f1b16286b0ffc331a787d74297593fa98c455fb24fcecf48d5be5bdeec48b6';
    const v2 = {
    ...
    expect(REJECTED_LIVE_CHARACTER_EXECUTION_DIGESTS).toContain(SUPERSEDED_V5_PYTHON_RUNTIME_DIGEST);
    ...
    expect(v1Auth.remainingBlockers).toContain('LIVE_AUTHORIZATION_V6_REQUIRED');
    ...
    expect(v2Auth.remainingBlockers).toContain('LIVE_AUTHORIZATION_V6_REQUIRED');
  ...
  it('binds V3 to the exact 1e29b0ba digest and refuses CREATE while the image cannot download Goat', () => {
    ...
    expect(pin.digest).toBe('sha256:78f1b16286b0ffc331a787d74297593fa98c455fb24fcecf48d5be5bdeec48b6');
    expect(pin.sourceCommit).toBe('db8658c835ccb0e5a3adc3201d7ba90e9aaeaa46');
