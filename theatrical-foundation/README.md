# Theatrical CGI Asset Foundation

Non-destructive look-dev and inventory for roadmap stage 3.
**Not approved. Not a start of Steps 9–16.**

| Document | Purpose |
| --- | --- |
| `ASSET_INVENTORY.md` | Production-ready / reusable / needs-upgrade / missing / prohibited |
| `THEATRICAL_CGI_REQUIREMENTS.md` | Measurable fidelity bars |
| `proposed/` | Proposed shader look-dev (outside `production-library/`) |
| `PREVIEW_INDEX.md` | Labeled stills for Justin |
| `reference/ACCESS_CHECK.md` | 22-image access check — reconstruction blocked until source files exist |

Canonical Pip, Goat, meadow, and map blends are unchanged.
The approved fingerprint remains `7876ac737de602578b67a8a20d85ea8a917c7ac4dac5e668f8bae37343e8f4b7`.
