...
const REQUIRED_AUTHORIZATION = 'DL_HOOK_01__PIP_ONE_PAID_ATTEMPT_NO_RETRY';
const TARGET_SEGMENT_ID = 'DL_HOOK_01__PIP';
const TARGET_CHARACTER_COUNT = 51;
const EXPECTED_BEFORE = { requests: 4, characters: 235 } as const;
const EXPECTED_AFTER = { requests: 5, characters: 286 } as const;
...
type SanitizedCanaryResult = {
  ...
  audioSha256: string;
  audioBytes: number;
  globalPaidRequests: 5;
  globalPaidCharactersUsed: 286;
  ep012SucceededRequests: 1;
  ep012SucceededCharacters: 51;
  reservations: 0;
  unfinalized: 0;
  failedAttempts: 0;
};
...
function assertBefore(preflight: Ep012NoProviderPreflight): void {
  ...
  if (
    ...
    preflight.ledger.ep012SucceededRequests !== 0 ||
    preflight.ledger.ep012SucceededCharacters !== 0 ||
    preflight.ledger.providerRequestsMade !== 0 ||
    preflight.ledger.storageVerifiedCount !== 0 ||
...
function assertAfter(preflight: Ep012NoProviderPreflight): void {
  ...
  if (
    ...
    preflight.ledger.ep012SucceededCharacters !== TARGET_CHARACTER_COUNT ||
