...
const REQUIRED_AUTHORIZATION = 'DL_DISCOVERY_01__PIP_ONE_PAID_ATTEMPT_NO_RETRY';
const TARGET_SEGMENT_ID = 'DL_DISCOVERY_01__PIP';
const TARGET_CHARACTER_COUNT = 80;
const EXPECTED_BEFORE = { requests: 6, characters: 320 } as const;
const EXPECTED_AFTER = { requests: 7, characters: 400 } as const;
...
type SanitizedCanaryResult = {
  ...
  audioSha256: string;
  audioBytes: number;
  globalPaidRequests: 7;
  globalPaidCharactersUsed: 400;
  ep012SucceededRequests: 3;
  ep012SucceededCharacters: 165;
  reservations: 0;
  unfinalized: 0;
  failedAttempts: 0;
};
...
function assertPriorGoatSucceeded(preflight: Ep012NoProviderPreflight): void {
  const prior = preflight.requestChecks.find((item) => item.segmentId === 'DL_HOOK_01__GOAT');
  if (
    !prior ||
    prior.characterCount !== 34 ||
    prior.ledgerEntryStatus !== 'SUCCEEDED' ||
    prior.eligibility !== 'ALREADY_SUCCEEDED' ||
    !prior.passed ||
    prior.blockers.length !== 0
  ) {
    fail('PRIOR_GOAT_SEGMENT_STATE_MISMATCH');
  }
}

function assertBefore(preflight: Ep012NoProviderPreflight): void {
  assertPriorPipSucceeded(preflight);
  assertPriorGoatSucceeded(preflight);
...
function assertAfter(preflight: Ep012NoProviderPreflight): void {
  ...
  if (
    ...
    preflight.ledger.ep012SucceededRequests !== 3 ||
    preflight.ledger.ep012SucceededCharacters !== 165 ||
    preflight.ledger.providerRequestsMade !== 3 ||
    preflight.ledger.storageVerifiedCount !== 3 ||
    check.characterCount !== TARGET_CHARACTER_COUNT ||
