#!/usr/bin/env tsx
/**
 * V6 zero-cost preflight. It performs only registry, R2 metadata/range, RunPod
...
async function inspectWorker(ref: string) {
  ...
  return {
    ...
    liveBlenderRuntime: labels['ddp.character.live.blender.runtime'] === 'true',
...
async function verifyLockedSource() {
  ...
  const evidenceKeys = [
    `tivvlejoy-assets/characters/${GOAT_CHARACTER_ID}/executions/${GOAT_V6_EXECUTION_ID}/manifest.json`,
    `jobs/${GOAT_V6_EXECUTION_ID}/status.json`,
    `jobs/${GOAT_V6_EXECUTION_ID}/startup-status.json`,
  ];
...
async function main() {
  ...
  if (pin.digest !== GOAT_V6_REQUIRED_DIGEST || !pin.ref) throw new Error('V6_PIN_MISMATCH');
  ...
  const exactName = pods.filter((pod) => pod.name === GOAT_V6_PLANNED_POD_NAME);
  const ledger = readGoatV6ConsumptionLedger(REPO_ROOT);
  ...
  const decision = evaluateGoatV6PaidExecutionAuthorization({
    repoRoot: REPO_ROOT,
    ...
    live: {
      ...
      registryBlenderVersion: registry.blender,
      registryStageCount: registry.stageCount,
      liveDepartmentUsesBlenderRuntime: registry.liveBlenderRuntime,
...
main().catch((error) => {
  const failed = {
    schema: 'TIVVLEJOY_GOAT_REAL_PAID_EXECUTION_V6_PREFLIGHT_V1',
