#!/usr/bin/env tsx
/**
 * Execute exactly one digest-bound V6 SECURE RTX 4090 Pod.
 *
...
import {
  ...
  GOAT_V6_MAX_RUNTIME_MINUTES,
  GOAT_V6_PLANNED_POD_NAME,
  GOAT_V6_REQUIRED_DIGEST,
  GOAT_V6_STARTUP_WATCHDOG_MINUTES,
  GOAT_V6_STOP_NEW_STAGES_MINUTES,
...
function hardRuntimeMs(hourlyRate: number): number {
  const absolute = GOAT_V6_MAX_RUNTIME_MINUTES * 60_000;
  if (!Number.isFinite(hourlyRate) || hourlyRate <= 0) return Math.min(absolute, 60_000);
  const costDerived = (GOAT_V6_HARD_COST_USD / hourlyRate) * 3_600_000 * 0.97;
...
async function persistRemoteConsumptionLedger(ledger: unknown): Promise<string> {
  ...
  const key = `tivvlejoy-assets/characters/${GOAT_CHARACTER_ID}/executions/${GOAT_V6_EXECUTION_ID}/launcher/consumption-ledger.json`;
  await client.send(
...
async function recoverExactNamedPod(client: RunpodClient): Promise<RunpodPodStatus | null> {
  ...
  while (Date.now() - started < AMBIGUOUS_CREATE_RECOVERY_MS) {
    const exact = (await listPods(client)).filter((pod) => pod.name === GOAT_V6_PLANNED_POD_NAME);
...
async function terminateWithConfirmation(client: RunpodClient, podIds: string[]) {
  ...
  while (Date.now() - started < TERMINATION_CONFIRM_MS) {
    try {
      ...
      const exact = finalPods.filter((pod) => pod.name === GOAT_V6_PLANNED_POD_NAME);
  ...
  return {
    confirmed: false,
    exactNamePods: finalPods.filter((pod) => pod.name === GOAT_V6_PLANNED_POD_NAME).length,
...
function validateOutputManifest(manifest: any) {
  ...
  if (manifest?.executionId !== GOAT_V6_EXECUTION_ID || manifest?.jobId !== GOAT_V6_EXECUTION_ID) {
...
async function run(): Promise<number> {
  ...
  const preflight = spawnSync(
    'pnpm',
    ['--filter', '@doodle-dash/web', 'exec', 'tsx', '../../scripts/cloud/goat-paid-execution-v6/preflight.ts'],
    {
      cwd: REPO_ROOT,
      encoding: 'utf8',
  ...
  if (!pin.ref || pin.digest !== GOAT_V6_REQUIRED_DIGEST) throw new Error('V6_PIN_CHANGED_AFTER_PREFLIGHT');
  ...
  if (before.filter(activePod).length !== 0 || before.some((pod) => pod.name === GOAT_V6_PLANNED_POD_NAME)) {
  ...
  const podEnv: Record<string, string> = {
    ...
    RENDER_JOB_ID: GOAT_V6_EXECUTION_ID,
    RENDER_WORKER_ID: `tivvlejoy-${GOAT_V6_EXECUTION_ID}`,
    CHARACTER_JOB_KIND: 'CHARACTER_MASTER_BUILD',
    CHARACTER_EXECUTION_MODE: 'live',
    CHARACTER_EXECUTION_ID: GOAT_V6_EXECUTION_ID,
    CHARACTER_PERSIST_ARTIFACTS: 'true',
    CHARACTER_WORKSPACE_DIR: `/tmp/tivvlejoy-character/${GOAT_V6_EXECUTION_ID}`,
    CHARACTER_ARTIFACT_DIR: `/tmp/tivvlejoy-character/${GOAT_V6_EXECUTION_ID}/artifacts`,
    GOAT_ALLOW_REAL_DOWNLOAD: 'true',
    GOAT_PERFORM_REAL_DOWNLOAD: 'true',
    PAID_EXECUTION_AUTHORIZED: 'true',
    PAID_AUTHORIZATION_JSON: JSON.stringify(receipt),
    AUTHORIZED_IMAGE_DIGEST: GOAT_V6_REQUIRED_DIGEST,
    AUTHORIZED_IMAGE_REF: pin.ref,
    RUNPOD_WORKER_IMAGE: pin.ref,
    DDP_IMAGE_DIGEST: GOAT_V6_REQUIRED_DIGEST,
    MAX_JOB_RUNTIME_MINUTES: String(GOAT_V6_MAX_RUNTIME_MINUTES),
    CHARACTER_STOP_NEW_STAGES_MINUTES: String(GOAT_V6_STOP_NEW_STAGES_MINUTES),
    STARTUP_WATCHDOG_MS: String(GOAT_V6_STARTUP_WATCHDOG_MINUTES * 60_000),
  ...
  if (process.env.GOAT_V6_LAUNCH_DRYRUN === '1') {
    log('dryrun_ok', {
      podName: GOAT_V6_PLANNED_POD_NAME,
      imageDigest: GOAT_V6_REQUIRED_DIGEST,
  ...
  const consumed = consumeGoatV6Authorization(REPO_ROOT);
  ...
  writeJson(
    LAUNCH_FILE,
    redact({
      schema: 'TIVVLEJOY_GOAT_REAL_PAID_EXECUTION_V6_LAUNCH_V1',
  ...
  try {
    ...
    if (!podId) {
      ...
      const startupKey = `jobs/${GOAT_V6_EXECUTION_ID}/startup-status.json`;
      const statusKey = `jobs/${GOAT_V6_EXECUTION_ID}/status.json`;
      for (;;) {
        ...
        if (!sawStartup && elapsedMs >= GOAT_V6_STARTUP_WATCHDOG_MINUTES * 60_000) {
        ...
        const stopMs = Math.min(GOAT_V6_STOP_NEW_STAGES_MINUTES * 60_000, Math.max(60_000, hardMs - 15 * 60_000));
        ...
        if (elapsedMs >= hardMs) {
          finalStatus = hardMs < GOAT_V6_MAX_RUNTIME_MINUTES * 60_000 ? 'HARD_COST_DEADLINE' : 'HARD_RUNTIME_DEADLINE';
    ...
    try {
      const exact = (await listPods(readClient)).filter((pod) => pod.name === GOAT_V6_PLANNED_POD_NAME);
  ...
  const finalReport = {
    schema: 'TIVVLEJOY_GOAT_REAL_PAID_EXECUTION_V6_FINAL_REPORT_V1',
    status: finalStatus,
    authorization: GOAT_LIVE_PAID_EXECUTION_AUTHORIZATION_V6,
    authorizationConsumed: true,
    executionId: GOAT_V6_EXECUTION_ID,
    workerImageDigestUsed: GOAT_V6_REQUIRED_DIGEST,
    ...
    withinCostAuthorization: estimatedCost <= GOAT_V6_HARD_COST_USD,
    cleanup: {
  ...
  writeJson(LAUNCH_FILE, redact({ ...finalReport, schema: 'TIVVLEJOY_GOAT_REAL_PAID_EXECUTION_V6_LAUNCH_V1' }));
  console.log(JSON.stringify(redact(finalReport), null, 2));
  if (!cleanup.confirmed || estimatedCost > GOAT_V6_HARD_COST_USD) return 3;
