...
import type { GoatV6AuthorizationReceipt } from '../../../apps/web/src/lib/tivvlejoy-character-source-intake/goat-v6-authorization';

export const REPO_ROOT = path.resolve(__dirname, '../../..');
export const OUT_DIR = path.join(REPO_ROOT, 'artifacts/tivvlejoy-goat-paid-execution-v6');
...
export function readIssuedAuthorization(): GoatV6AuthorizationReceipt {
  if (!existsSync(AUTHORIZATION_FILE)) throw new Error('V6_AUTHORIZATION_NOT_ISSUED');
  const value = readJson<GoatV6AuthorizationReceipt>(AUTHORIZATION_FILE);
  if (value.authorizationName !== 'TIVVLEJOY_GOAT_REAL_PAID_EXECUTION_AUTHORIZATION_V6') {
    throw new Error('V6_AUTHORIZATION_NAME_MISMATCH');
  }
  return value;
}
