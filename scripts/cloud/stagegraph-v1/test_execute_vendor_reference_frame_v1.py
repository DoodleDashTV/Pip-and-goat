...
class VendorReferenceExecutorTest(unittest.TestCase):
    ...
    def test_consumed_auth_and_visual_reject_block_another_create(self):
        ...
        self.assertEqual(auth["authorizationSha256"], MODULE.AUTH_SHA256)
        self.assertEqual(auth["authorizationSha256"], "12618427bd4c083d50c5affb7f13fafa032061a465dec3f42e3793eb4abbd031")
        ...
        self.assertEqual(status["paidCreateCount"], 3)
        self.assertIsNone(status["humanVisualDecision"])
        self.assertFalse(status["freshPaidAuthorizationRequired"])
        self.assertFalse(status["anotherVendorReferenceFrameRequired"])
        self.assertTrue(status["freshVendorReferenceAuthorizationPresent"])
        self.assertTrue(status["rejectedVendorReferenceIneligible"])
        self.assertEqual(status["vendorReferenceImageSha256"], "e873e483e817962afde700dcf4072655736f5dde271b2723a1ebdfc9046f12ad")
        self.assertEqual(status["rejectedVendorReferenceImageSha256"], "6f31cb689488813d54608aff0b0c959835204fb54f62e1d2e321d3957827c3b2")
        self.assertEqual(status["previousRejectedVendorReferenceImageSha256"], "a1276acb73ada320240cced525dc9902ff89516da97c019bc87c334a94cce400")
        ...
        decision_v2 = json.loads((REPO / "artifacts/tivvlejoy-stagegraph-v1/VENDOR_REFERENCE_VISUAL_REVIEW_DECISION_V2.json").read_text())
        self.assertEqual(decision_v2["imageSha256"], "6f31cb689488813d54608aff0b0c959835204fb54f62e1d2e321d3957827c3b2")
        self.assertEqual(decision_v2["decision"], "REJECTED")
        self.assertFalse(decision_v2["vendorReferenceReproducedApproved"])
        ...
        self.assertNotEqual(request["proposedAuthorizationCoreSha256"], auth["authorizationSha256"])
        consumed_v2 = json.loads((REPO / "artifacts/tivvlejoy-stagegraph-v1/VENDOR_REFERENCE_AUTHORIZATION_CONSUMED_V2.json").read_text())
        self.assertEqual(consumed_v2["authorizationSha256"], "270865630a301bf39d7067b0545c56a489d37c77c0633dc605d2d95ff7934161")
        self.assertTrue(consumed_v2["consumed"])
