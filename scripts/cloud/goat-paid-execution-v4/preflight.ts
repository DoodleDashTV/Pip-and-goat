#!/usr/bin/env tsx
/**
 * V4 zero-cost preflight. It performs only registry, R2 metadata/range, RunPod
...
async function verifyLockedSource() {
  ...
  const evidenceKeys = [
    `tivvlejoy-assets/characters/${GOAT_CHARACTER_ID}/executions/${GOAT_V4_EXECUTION_ID}/manifest.json`,
    `jobs/${GOAT_V4_EXECUTION_ID}/status.json`,
    `jobs/${GOAT_V4_EXECUTION_ID}/startup-status.json`,
  ];
...
async function main() {
  ...
  if (pin.digest !== GOAT_V4_REQUIRED_DIGEST || !pin.ref) throw new Error('V4_PIN_MISMATCH');
  ...
  const exactName = pods.filter((pod) => pod.name === GOAT_V4_PLANNED_POD_NAME);
  const ledger = readGoatV4ConsumptionLedger(REPO_ROOT);
  ...
  const decision = evaluateGoatV4PaidExecutionAuthorization({
    repoRoot: REPO_ROOT,
...
main().catch((error) => {
  const failed = {
    schema: 'TIVVLEJOY_GOAT_REAL_PAID_EXECUTION_V4_PREFLIGHT_V1',
