#!/usr/bin/env tsx
/** Issue the exact digest-bound V5 receipt. Creates no Pod and performs no download. */
import {
...
function main(): void {
  if (/^sha256:0{64}$/.test(GOAT_V5_REQUIRED_DIGEST)) {
    throw new Error('V5_IMAGE_DIGEST_NOT_PINNED');
  }
