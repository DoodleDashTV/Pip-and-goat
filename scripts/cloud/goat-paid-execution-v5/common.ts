...
import type { GoatV5AuthorizationReceipt } from '../../../apps/web/src/lib/tivvlejoy-character-source-intake/goat-v5-authorization';

export const REPO_ROOT = path.resolve(__dirname, '../../..');
export const OUT_DIR = path.join(REPO_ROOT, 'artifacts/tivvlejoy-goat-paid-execution-v5');
...
export function readIssuedAuthorization(): GoatV5AuthorizationReceipt {
  if (!existsSync(AUTHORIZATION_FILE)) throw new Error('V5_AUTHORIZATION_NOT_ISSUED');
  const value = readJson<GoatV5AuthorizationReceipt>(AUTHORIZATION_FILE);
  if (value.authorizationName !== 'TIVVLEJOY_GOAT_REAL_PAID_EXECUTION_AUTHORIZATION_V5') {
    throw new Error('V5_AUTHORIZATION_NAME_MISMATCH');
  }
  return value;
}
