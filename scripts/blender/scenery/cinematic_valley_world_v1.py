...
from cinematic_hero_rebuild_v2 import (  # noqa: E402
    ...
    apply_light_mood as apply_v2_light_mood,
    import_nature_library,
    COMP_CAMERAS as V2_COMP_CAMERAS,
)
...
def _meadow_zone_points(cx: float, cy: float, rx: float, ry: float, count: int, seed: float):
    """Irregular disk samples. Not a grid, not a duplicated patch."""
    points = []
    for i in range(count):
        ang = i * 2.399963 + seed
        radius = math.sqrt((i + 0.31) / max(1, count))
        radius *= 0.82 + 0.18 * math.sin(i * 1.73 + seed)
        x = cx + radius * rx * math.cos(ang) + 0.22 * rx * math.sin(i * 2.11 + seed)
        y = cy + radius * ry * math.sin(ang) + 0.20 * ry * math.cos(i * 1.67 + seed * 1.4)
        points.append((x, y))
    return points


def _meadow_keep(x: float, y: float) -> bool:
    if in_river_channel(x, y, margin=1.45):
        return False
...
def cinematic_meadow_material(image) -> bpy.types.Material:
    ...
    ramp.color_ramp.elements[0].color = (0.034, 0.062, 0.024, 1.0)
    ramp.color_ramp.elements[1].position = 0.84
    ramp.color_ramp.elements[1].color = (0.072, 0.110, 0.038, 1.0)
    mid = ramp.color_ramp.elements.new(0.50)
    mid.color = (0.050, 0.082, 0.030, 1.0)
    ...
    if image is not None:
        ...
        mapping = nodes.new("ShaderNodeMapping")
        mapping.inputs["Scale"].default_value = (0.022, 0.022, 0.022)
    ...
    detail_ramp.color_ramp.elements[0].color = (0.028, 0.048, 0.018, 1.0)
    detail_ramp.color_ramp.elements[1].color = (0.080, 0.105, 0.038, 1.0)
    links.new(detail.outputs["Fac"], detail_ramp.inputs["Fac"])
    detail_mix = _mix_rgb(nodes)
    if "Color1" in detail_mix.inputs:
        detail_mix.inputs["Fac"].default_value = 0.22
    ...
    # Sparse earth islands inside some cells, not a dotted carpet.
    soil = nodes.new("ShaderNodeTexNoise")
    soil.inputs["Scale"].default_value = 0.012
    if "Detail" in soil.inputs:
        soil.inputs["Detail"].default_value = 1.5
    links.new(coord.outputs["Object"], soil.inputs["Vector"])
    soil_fac = nodes.new("ShaderNodeMapRange")
    soil_fac.inputs["From Min"].default_value = 0.68
    soil_fac.inputs["From Max"].default_value = 0.86
    soil_fac.inputs["To Min"].default_value = 0.0
    soil_fac.inputs["To Max"].default_value = 0.70
    ...
    if "Color1" in soil_mix.inputs:
        ...
        soil_mix.inputs["Color2"].default_value = (0.155, 0.095, 0.046, 1.0)
    ...
    for i, center in enumerate(centers):
        ...
        for col, offset in enumerate(offsets):
            ...
            if abs(offset) > 0.62:
                ...
                if foam_edges and HERO_X_MIN <= center.x <= HERO_X_MAX and offset < 0.0:
                    # Keep the hero south seal. Lateral noise here opened the V42 void.
                    edge_noise = 0.05 * math.sin(i * 0.37)
                elif foam_edges:
...
def _hero_zone_material(kind: str) -> bpy.types.Material:
    mat = dirt_bank_material()
    colors = {
        "wet": ((0.058, 0.042, 0.024), 0.82, 0.10),
        "damp": ((0.108, 0.072, 0.038), 0.92, 0.05),
        "soil": ((0.155, 0.100, 0.050), 0.96, 0.04),
        "grass": ((0.044, 0.078, 0.028), 0.98, 0.03),
        "gravel": ((0.138, 0.118, 0.090), 0.86, 0.08),
    }
...
def _hero_bank_runs(kind: str, event: dict) -> tuple[float, float, float, float]:
    """Wet / damp / soil / grass runs from the shared water edge, in metres."""
    if kind == "overhang":
        return 0.08, 0.16, 0.34, 0.52
    if kind == "inlet":
        return 0.10, 0.20, 0.48, 0.04
    if kind == "shelf":
        return 1.20, 1.95, 2.45, 0.06
    if kind == "retreat":
        return 0.20, 1.35, 1.95, 0.05
    if kind == "rock":
        return 0.14, 0.36, 0.62, 0.08
    if kind == "gravel":
        return 0.72, 1.20, 1.65, 0.03
    if kind == "bay":
        return 1.05, 1.65, 2.10, 0.03
    if kind == "cut":
        return 0.10, 0.28, 0.62, 0.06
    return (
        max(0.08, event.get("wet", 0.20)),
        max(0.16, event.get("damp", 0.45)),
        max(0.34, event.get("soil", 0.80)),
        max(0.03, event.get("grass", 0.10)),
    )
...
def place_hero_damp_shelves(centers: list[Vector]) -> list:
...
def setup_mist_and_compositor() -> None:
    ...
    if hasattr(scene.world, "mist_settings"):
        ...
        scene.world.mist_settings.start = 34.0
...
def main() -> int:
    ...
    for prop in place_meadow_ecology():
    ...
    nature_members = []
    nature_library = {}
    # V3 uses original EcoKit .blend trees, not the Nature FBX card kit.
    ...
    proof = {
        ...
        "lighting": "cinematic_hero_v3_daylight",
        "groundSource": "tj_hero_v3_authored_terrain",
        "heroRebuild": "v3_source_fidelity",
    ...
    if str(args.v3_compare or "").lower() == "comps":
        scene = bpy.context.scene
        ...
        for key, spec in COMP_CAMERAS.items():
            ...
            scene.camera = cam
            bpy.context.view_layer.update()
            scene.render.filepath = str(out / f"v3_comp_{key.lower()}_")
            write_progress("V3_COMP", id=key, camera=cam.name, lens=spec["lens"])
            bpy.ops.render.render(write_still=True)
        write_progress("LOOKDEV_COMPLETE", compare="v3_comps", frames=3)
        return 0
    if str(args.v3_compare or "").lower() == "lights":
        scene = bpy.context.scene
        scene.render.use_persistent_data = False
        scene.frame_set(210)
        for marker in scene.timeline_markers:
            marker.camera = None
        key = str(args.v3_camera or "A").upper()
    ...
    if str(args.v2_compare or "").lower() == "comps":
        ...
        for marker in scene.timeline_markers:
            marker.camera = None
        for key, spec in V2_COMP_CAMERAS.items():
            cam = bpy.data.objects.get(spec["name"])
            if cam is None:
                continue
            scene.camera = cam
    ...
    if str(args.v2_compare or "").lower() == "lights":
        ...
        spec = V2_COMP_CAMERAS.get(key) or V2_COMP_CAMERAS["A"]
        cam = bpy.data.objects.get(spec["name"])
        if cam is not None:
            scene.camera = cam
        for mood in ("A", "B", "C"):
            apply_v2_light_mood(mood)
