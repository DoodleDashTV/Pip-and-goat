"""Locked-camera cinematic lighting proof on passed ground-detail dressing."""
...
def parse_args():
    ...
    parser.add_argument("--proof-name", default="FOREST_CINEMATIC_LIGHTING_CAMERA_PROOF_V1.png")
...
def render_still(scene, path: Path) -> dict:
    ...
    if not path.is_file():
        raise RuntimeError("CINEMATIC_PROOF_MISSING:" + path.name)
...
def main():
    ...
    receipt = {
        "schema": "TIVVLEJOY_FOREST_CINEMATIC_LIGHTING_PROOF_V1",
        "result": "RENDERED",
        "paidCreateCount": 0,
        "paidSpendUsd": 0,
