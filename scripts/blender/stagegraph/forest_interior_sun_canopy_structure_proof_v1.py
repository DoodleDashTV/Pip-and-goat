"""Locked-camera interior-sun and canopy-structure proof."""
...
def parse_args():
    ...
    parser.add_argument("--proof-name", default="FOREST_INTERIOR_SUN_CANOPY_STRUCTURE_PROOF_V1.png")
...
def render_still(scene, path: Path) -> dict:
    ...
    if not path.is_file():
        raise RuntimeError("INTERIOR_SUN_PROOF_MISSING:" + path.name)
...
def _prepare(args):
    ...
    scene.cycles.samples = max(int(args.samples), 12)
...
def main():
    ...
    cinematic = apply_cinematic_lighting_recovery(scene)
    interior = apply_interior_sun_canopy_structure(scene)
    out_path = out_dir / args.proof_name
    still = render_still(scene, out_path)
    receipt = {
        "schema": "TIVVLEJOY_FOREST_INTERIOR_SUN_CANOPY_STRUCTURE_PROOF_V1",
        "result": "RENDERED",
        "paidCreateCount": 0,
        "paidSpendUsd": 0,
        "samples": scene.cycles.samples,
        "materialLightingLocked": LOCKED_MATERIAL_LIGHTING,
        "rainbowSpeckObjectsHidden": prepared["rainbowSpeckObjectsHidden"],
        "groundDetail": prepared["groundDetail"],
        "canopyRepair": canopy,
        "cinematicRecovery": cinematic,
        "interiorRecovery": interior,
        "still": still,
