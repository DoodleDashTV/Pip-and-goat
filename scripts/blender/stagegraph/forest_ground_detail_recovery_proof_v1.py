"""Locked-camera rainbow object-ID and ground-detail proofs."""
...
def parse_args():
    ...
    parser.add_argument("--mode", choices=("object-id", "production"), default="object-id")
    parser.add_argument("--proof-name", default="FOREST_GROUND_DETAIL_CAMERA_PROOF_V1.png")
    parser.add_argument("--hide-classes", default="prod_flower,ecokit_floral")
...
def render_still(scene, path: Path) -> dict:
    ...
    if not path.is_file():
        raise RuntimeError("GROUND_DETAIL_PROOF_MISSING:" + path.name)
    return {
        "path": str(path),
...
def _prepare(args):
    ...
    scene.cycles.samples = max(int(args.samples), 6)
    ...
    lighting = apply_locked_material_lighting(scene)
...
def main():
    ...
    if args.mode == "object-id":
        painted = paint_rainbow_object_ids(scene)
        out_path = out_dir / "FOREST_RAINBOW_SPECK_OBJECT_ID_V1.png"
        still = render_still(scene, out_path)
        receipt = {
            "schema": "TIVVLEJOY_FOREST_RAINBOW_SPECK_OBJECT_ID_PROOF_V1",
            "result": "RENDERED",
            "paidCreateCount": 0,
            "paidSpendUsd": 0,
            ...
            "painted": painted,
            "still": still,
            "lightingLocked": LOCKED_MATERIAL_LIGHTING,
            "cameraChanged": False,
        }
        (out_dir / "FOREST_RAINBOW_SPECK_OBJECT_ID_V1.json").write_text(
    ...
    classes = tuple(item.strip() for item in args.hide_classes.split(",") if item.strip())
    hidden = hide_identified_rainbow_specks(scene, classes)
    detail = replace_failed_micro_dressing(scene)
    apply_locked_material_lighting(scene)
    out_path = out_dir / args.proof_name
    still = render_still(scene, out_path)
    receipt = {
        "schema": "TIVVLEJOY_FOREST_GROUND_DETAIL_PROOF_V1",
        "result": "RENDERED",
        "paidCreateCount": 0,
        "paidSpendUsd": 0,
        "samples": scene.cycles.samples,
    ...
    print(json.dumps({
        ...
        "path": still["path"],
        "sha256": still["sha256"],
        "hidden": [item["name"] for item in hidden],
        "stampsHidden": detail.get("stampsHidden"),
